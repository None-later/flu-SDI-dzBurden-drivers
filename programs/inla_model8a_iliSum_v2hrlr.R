...
## Name: Elizabeth Lee
## Date: 12/30/17
## Function: Model 8a v2: high and low risk states only; non-spatial model w/o region effects, shifted response variable (e.g., seasonal intensity 0 -> 1), with interaction terms
...
# single code
modCodeLs <- c("8a_iliSum_v2-6_hr", "8a_iliSum_v2-6_lr") 
keepLs <- list(c("08", "13", "24", "26", "37", "45", "46", "51"), c("12", "20", "27", "29", "30", "35", "49"))
...
for (i in 1:length(modCodeLs)){
  ...
  ################################
  keep_hrlrStates <- function(full_df, stLs){
    # keep specific states that are high or low risk
    print(match.call())
    
    # return original dataframe including only states in stLs
    full_df2 <- full_df %>%
      filter(fips_st %in% stLs)
