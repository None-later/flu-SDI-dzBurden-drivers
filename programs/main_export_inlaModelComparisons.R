...
pairLs <- c("8a_iliSum_v2-6", "8a_iliSum_v2-6_rmFixed")
abbrLs <- gsub("_iliSum_v", "V", pairLs)
pair_plotFormats <- list(w = 10, h = 5, descrip = paste(abbrLs[1], abbrLs[2], sep = "_"), lvls = pairLs, labs = abbrLs)
pair_datFormats <- list(fit_dataType = "summaryStats", refModCode = "8a_wksToEpi_v2-6") # fit_dataType = "summaryStats", "posteriorSamples"
