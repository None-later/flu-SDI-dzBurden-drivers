...
cleanR_iliSum_shift1_irDt_cty <- function(filepathList){
  # clean response variable: irDt.sum plus 1; 10/12/17
  print(match.call())
  ...
  # 7/18/16: add incl.analysis indicator
  # grab disease burden metric (e.g., ilinDt): match "ir" 1+ times
  dbCode <- grep("ir+", strsplit(filepathList$path_response_cty, "_")[[1]], value=T)
  # clean burden data
  iliSum_data <- read_csv(filepathList$path_response_cty, col_types = "icllcd") %>%
    filter(metric == sprintf("%s.sum", dbCode)) %>%
    select(-metric) %>%
...
cleanR_iliSum_shift1_cty_aggBias <- function(filepathList){
  # clean response variable: ilinDt.sum plus 1; 12/15/16 in preparation for aggBias modeling
  print(match.call())
...
cleanR_iliSum_2009p_shift1_cty <- function(filepathList){
  # clean response variable: ilinDt.sum plus 1; 4/20/17 pandemic season only
  print(match.call())
...
cleanR_iliExcessBL_shift1_cty <- function(filepathList){
  # 3/2/17 clean response variable: ilinDt.excess.BL for validation checks
  print(match.call())
  ...
  return_data <- full_join(iliExcessBL_data, pop_data, by = c("season", "fips")) %>%
    select(fips, county, st, stateID, lat, lon, season, year, pop, y, has.epi, incl.analysis) %>%
    mutate(y1 = y+1) %>% # add 1 to all seasonal intensity values
    group_by(season) %>%
    mutate(E = weighted.mean(y1, pop, na.rm = TRUE)) %>%
    ungroup %>%
    filter(season >= 3 & season <= 9) # 12/12/16
...
cleanX_priorBurden_irDt_cty <- function(filepathList){
  # clean data variable: irDt.sum for previous year; new 10/12/17
  print(match.call())
...
##########################################
cleanX_protectedFromPrevSeason_irDt_cty <- function(filepathList){
  # irDt version: clean variable indicating protection conferred from infection during previous flu season: incorporates previous and current season subtype/type distributions (H1/H3/B) and strain similarity
  print(match.call())
  
  priorburdenDat <- cleanX_priorBurden_irDt_cty(filepathList) %>% mutate(fips_st = substr(fips, 1, 2)) # fips, season, priorBurden
  protectedPropDat <- cleanX_multsrcSubtypeDistrStrainSim_reg() # season, region, estImmuneProp
  cw <- cleanX_cdcFluview_H3_region() %>% select(region, fips) %>% rename(fips_st = fips) %>% distinct # region, fips_st
