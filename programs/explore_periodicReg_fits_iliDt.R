...
#### plot formatting ################################
w <- 9; h <- 6
num <- 6 # number of figures per plot
...
for(i in indexes){
  dummyplots <- ggplot(data_plot %>% filter(for.plot>= i & for.plot < i+num), aes(x=week, y=ili.dt, group=zipname)) +
  ...
  ziplabels <- data_plot %>% select(zipname) %>% distinct %>% slice(c(i, i+num-1)) 
  ggsave(sprintf("periodicReg_%sfits_iliDt%s%s_%s-%s.png", code, code2, code.str, ziplabels[1,], ziplabels[2,]), dummyplots, width=w, height=h)
} # saved 10/20/15
...
for (i in indexes2){
  dummyplots <- ggplot(data_plot2 %>% filter(for.plot>= i & for.plot < i+num), aes(x=.fitted, y=resid, group=zipname)) +
