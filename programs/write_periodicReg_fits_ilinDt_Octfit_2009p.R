...
## Name: Elizabeth Lee
## Date: 4/17/17
## Function: Rerun data processing to accomodate 2009 pandemic timeframe. perform periodic regression on the detrended relative ilin (ilinDt)
...
write_periodicReg_fits_ilinDt_Octfit <- function(span.var, degree.var, spatial){
  ...
  # spatial <- list(scale = "county", stringcode = "County", stringabbr = "_cty", serv = "_totServ", servToggle = "", panToggle = "_2009p")
  ...
  if (spatial$scale == 'county'){
    ILI_full_df <- read_csv(file=sprintf('loess%s_all%sMods_ILIn%s%s%s.csv', code.str, spatial$stringcode, spatial$servToggle, spatial$ageToggle, spatial$panToggle), col_types=list(fips = col_character(), ili = col_double(), pop = col_integer(),  ILIn = col_double(), .fitted=col_double(), .se.fit=col_double(), ilin.dt=col_double(), ILIn = col_double())) %>%
      rename(scale = fips, .fittedLoess = .fitted, .se.fitLoess = .se.fit)
  }
  
  #### process data for periodic regression ####################################
  # create new data for augment
  newbasedata <- ILI_full_df %>% select(Thu.week, t) %>% unique %>% filter(Thu.week < as.Date('2010-05-01'))
  
  # perform periodic regression
  print('performing periodic regression')
  allMods <- ILI_full_df %>% filter(fit.week) %>% filter(Thu.week < as.Date('2010-05-01')) %>% 
  ...
  allMods_fit_ILI <- right_join((allMods_aug %>% select(-t)), (ILI_full_df %>% filter(Thu.week < as.Date('2010-05-01'))), by=c('Thu.week', 'scale')) %>% 
  ...
  write.csv(perReg, file=sprintf('periodicReg_%sall%sMods_ilinDt%s%s%s%s%s.csv', code, spatial$stringcode, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str), row.names=FALSE)
  # write tidy coefficient dataset
  write.csv(tidCoef, file=sprintf('tidyCoef_periodicReg_%sall%sMods_ilinDt%s%s%s%s%s.csv', code, spatial$stringcode, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str), row.names=FALSE)
  # write summary statistics for all models
  write.csv(sumStats, file=sprintf('summaryStats_periodicReg_%sall%sMods_ilinDt%s%s%s%s%s.csv', code, spatial$stringcode, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str), row.names=FALSE)
