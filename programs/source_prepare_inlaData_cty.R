...
model8a_iliSum_irDt_v7 <- function(filepathList){
  # 10/12/17 model 8a_v7 -- total population
  # multi-season model, shifted1 iliSum response, all sampling effort, and driver variables, based on irDt
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = log(response+1), E = expected response+1
  print(match.call())
  print(filepathList)
  ...
  #### import data ####
  # IMS Health based tables
  mod_cty_df <- cleanR_iliSum_shift1_irDt_cty(filepathList)
  imsCov_cty_df <- cleanO_imsCoverage_cty()
  imsCareseek_cty_df <- cleanO_imsCareseekTot_cty() # 1/5/17 visitsPerPop from sdi flu data
  # all county tables
  sahieIns_cty_df <- cleanO_sahieInsured_cty()
  ...
  cdcH3A_df <- cleanX_cdcFluview_H3A_region()
  cdcB_df <- cleanX_cdcFluview_B_region() %>% select(-region)
  protectedPriorSeas_df <- cleanX_protectedFromPrevSeason_irDt_cty(filepathList)
  # graph index IDs
  graphIdx_df <- clean_graphIDx(filepathList, "county")
  graphIdx_st_df <- clean_graphIDx(filepathList, "state")
...
model8e_epiDur_log_v7 <- function(filepathList){
  # 1/13/17 model 8e_v7 -- total population
  # multi-season model, epiDur count response in weeks, all sampling effort, and driver variables
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = non-zero response, E = expected response
...
model8e_epiDur_irDt_v7 <- function(filepathList){
  # 10/12/17 model 8e_v7 -- total population
  # multi-season model, epiDur count response in weeks, all sampling effort, and driver variables, irDt version
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = non-zero response, E = expected response
  print(match.call())
  print(filepathList)
...
model9a_iliSum_2009p_v7 <- function(filepathList){
  # 4/20/17 model 9a_v7 -- total population, pandemic data processing
  # single season models, shifted1 iliSum response, all sampling effort, and driver variables
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = log(response+1), E = expected response+1
  print(match.call())
  ...
  # all state tables
  childVax_st_df <- cleanX_nhfsBRFSSChildVaxCov_st_2009p()
  adultVax_st_df <- cleanX_nhfsBRFSSAdultVaxCov_st_2009p()
  # grab regionID
  ...
  full_df <- full_join(dummy_df2, saipePov_cty_df, by = c("year", "fips")) %>%
    ...
    mutate(X_popdensity = centerStandardize(popDensity)) %>%
    mutate(X_housdensity = centerStandardize(housDensity)) %>%
    mutate(X_vaxcovC = centerStandardize(childMonoVax)) %>%
    mutate(X_vaxcovA = centerStandardize(adultMonoVax)) %>%
    mutate(X_priorImmunity = centerStandardize(protectionPrevSeason)) %>%
    mutate(X_humidity = centerStandardize(humidity)) %>%
    mutate(X_pollution = centerStandardize(avg_pm)) %>%
    mutate(X_singlePersonHH = centerStandardize(perc_hh_1p)) %>%
    ...
    filter(!is.na(graphIdx_st)) %>% # rm data not in graph
    mutate(logE = log(E), y1 = log(y1)) %>% # model response y1 = log(y+1)
    select(-stateID, -adjProviderCoverage, -visitsPerPopT, -insured, -poverty, -child, -adult, -hospitalAccess, -popDensity, -housDensity, -childMonoVax, -adultMonoVax, -protectionPrevSeason, -humidity, -avg_pm, -perc_hh_1p) %>%
    filter(season == 9) %>%
    mutate(season = 10) %>%
    mutate(ID = seq_along(fips))
...
model8a_iliSum_regions_raw <- function(filepathList, regionList){
  # 4/16/17 model 8a iliSum regions (raw data) 
  # multi-season model, shifted1 iliSum response, all sampling effort, and driver variables
  # with 2 spatial terms - state flight passenger, county neighbor
  # y1 = log(response+1), E = expected response+1
  print(match.call())
  ...
  full_df <- full_join(dummy_df2, saipePov_cty_df, by = c("year", "fips")) %>%
    ...
    rename(rX_priorImmunity = protectionPrevSeason) %>%
