...
## Name: Elizabeth Lee
## Date: 10/20/15
## Function: explore ways to define the flu season with ili.dt metric
...
#### data import ####################################
setwd('../R_export')
data <- read_csv(file=sprintf('periodicReg_%sallZip3Mods_iliDt%s%s.csv', code, code2, code.str), col_types=list(ili=col_double(), .fitted=col_double(), .se.fit=col_double(), zipname=col_character(), ili.dt=col_double(), .fittedLoess=col_double(), .se.fitLoess=col_double()))
...
#### Analysis 1  ###########################
# Analysis 1: a zip3 had a flu epidemic if ILI.dt > epi.threshold for at least x weeks during the flu period
# identify zip3s with ILI.dt > epi.threshold for at least 4 weeks during a flu.week == TRUE (not necessarily consec)
zips.over.thresh_ct <- data3 %>% filter(flu.week) %>% group_by(season, zipname) %>% summarise(., num.epi.weeks = sum(ili.dt>epi.thresh, na.rm=TRUE)) %>% mutate(incl.analysis = num.epi.weeks >= 4)
# compare expectation to zip3 = 013
test <- data3 %>% filter(zipname=='013', season==1, flu.week) %>% select(ili.dt, epi.thresh) %>% mutate(ili.dt>epi.thresh)
...
zips.over.thresh_flu_consec <- data3 %>% filter(flu.week) %>% mutate(epidemic = ili.dt>epi.thresh) %>% group_by(season, zipname) %>% summarise(flu = rle.func(epidemic))
# maximum number of consecutive epidemics during non-flu weeks
zips.over.thresh_nonflu_consec <- data3 %>% filter(!flu.week) %>% mutate(epidemic = ili.dt>epi.thresh) %>% group_by(season, zipname) %>% summarise(nf = rle.func(epidemic))
...
dir.create(sprintf('../graph_outputs/explore_fluSeasonDefinition_%siliDt%s%s', code, code2, code.str), showWarnings=FALSE)
setwd(sprintf('../graph_outputs/explore_fluSeasonDefinition_%siliDt%s%s', code, code2, code.str))
...
ggsave(sprintf("consecEpiWeeks_%siliDt%s%s.png", code, code2, code.str), consec.flu.nf.single, width=4, height=4)
...
ggsave(sprintf("consecEpiWeeks_fluPeriod_%siliDt%s%s.png", code, code2, code.str), consec.flu, width=9, height=6)
# nonflu period plot
...
ggsave(sprintf("consecEpiWeeks_nfPeriod_%siliDt%s.png", code, code2, code.str), consec.nflu, width=9, height=6)
...
# span0.6: during flu period: 75% of zip3-season combinations have 4+ consecutive weeks above the epidemic threshold
# span0.6 during non-flu period: 75% of zip3-season combinations have 4 or fewer consecutive weeks above the epidemic threshold
# span0.4: during flu period: 75% of zip3-season combinations have 4+ consecutive weeks above the epidemic threshold
# span0.4: during non-flu period: 80% of zip3-season combinations have 4 or fewer consecutive weeks above the epidemic threshold
...
consec.flu.nf.single2 <- ggplot(zips.over.thresh_consec2, aes(x=consec.weeks, group=period)) +
  geom_histogram(aes(y=..density.., color=period), binwidth=1, alpha=0.5, position="dodge") +
  coord_cartesian(ylim=c(0, 0.3))
ggsave(sprintf("consecEpiWeeks_%siliDt%s%s_rmNoise.png", code, code2, code.str), consec.flu.nf.single2, width=4, height=4)
