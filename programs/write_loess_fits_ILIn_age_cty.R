...
#     degree.var <- 2
  ...
  # import zip3 data
  zipILI_df <- read_csv(sprintf('ilicByallZip3_allWeekly%s%s.csv', spatial$serv, spatial$age), col_types = list(zip3 = col_character(), ili = col_integer(), cov_z.y = col_double(), alpha_z.y = col_double(), ILIc = col_double(), cov_below5 = col_logical())) %>%
