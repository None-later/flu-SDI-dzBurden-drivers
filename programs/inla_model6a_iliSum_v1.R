...
modCodeStr <- "6a_iliSum_modCheck"; testDataOn <- TRUE
seasons <- 4:4 
rdmFx_RV <- "nu"
...
for (s in seasons){
  ...
  mod <- inla(formula, 
              ...
              control.mode = list(result = starting4, restart = TRUE), # http://www.r-inla.org/?place=msg%2Fr-inla-discussion-group%2Fuf2ZGh4jmWc%2FA0rdPE5W7uMJ
              verbose = TRUE, 
