...
explore_periodicReg_fits_ilicnDt <- function(span.var, degree.var){
  ...
  data_plot2 <- data_plot %>% filter(fit.week) %>% mutate(resid = ilicn.dt - .fitted) %>% mutate(data95indic = ifelse(ilicn.dt > .fitted-(1.96*.se.fit) & ilicn.dt < .fitted+(1.96*.se.fit), TRUE, FALSE))
  indexes2 <- indexes
