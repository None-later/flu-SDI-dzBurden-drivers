...
modCodeStr <- "6a_iliSum_v2testing1"; testDataOn <- TRUE
seasons <- 2:4
rdmFx_RV <- "nu"
...
for (s in seasons){
  ...
  mod <- inla(formula, 
              ...
              control.predictor = list(compute = TRUE, link = c(rep(1, nrow(modData_full)))), # compute summary statistics on fitted values, link designates that NA responses are calculated according to the first likelihood for the first (nrow(modData_full)) rows
