...
## Name: Elizabeth Lee
## Date: 2/23/16
## Function: INLA Model 3h - covariate, sampling effort, & spatial CAR model
...
formula <- logy ~ -1 + f(ID, model = "besag", graph = path_adjMxExport_st) + I(O_imscoverage) + I(O_careseek) + I(O_insured) + I(X_poverty) # v1: poverty
...
for (s in seasons){
  ...
  # plot marginal posteriors for driver & sampling coefficients
  ...
  #### INLA CSV file export - by season ####
