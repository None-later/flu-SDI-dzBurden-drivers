...
## Name: Elizabeth Lee
## Date: 11/1/15
## Function: write fullIndic data for CA zip3s with and without filtering for incl.analysis
...
data6 <- left_join(data4, (data5 %>% ungroup %>% select(Thu.week, zip3, in.season)), by = c("Thu.week", "zip3")) %>% 
  mutate(Thu.week=as.Date(Thu.week, origin="1970-01-01")) %>%
  filter(zip3 %in% zipset)
  

# save to file 
setwd('./dataSnapshot_NIHbigdata')
print(sprintf('writing full indicators to file %s', code.str))
write.csv(data6, file = sprintf('fullIndicAll_CA_periodicReg_%silicnDt%s%s_analyzeDB.csv', code, code2, code.str), row.names=FALSE)
# 11/1/15
