...
## Name: Elizabeth Lee
## Date: 10/17/16
## Function: Model 7a debug v3 gamma model without offsets
...
modCodeStr <- "7a_debug_v3-12"; testDataOn <- TRUE
rdmFx_RV <- "nu"
...
if (testDataOn){
  ...
  formula <- Y ~ -1 + 
    ...
    intercept_nonzero + O_imscoverage_nonzero 
} else{
#### Import and process data ####
  ...
  formula <- Y ~ -1 + 
    ...
    f(regionID_nonzero, model = "iid") + 
    f(season_nonzero, model = "iid") +
    intercept_nonzero + O_imscoverage_nonzero + O_careseek_nonzero + O_insured_nonzero + X_poverty_nonzero + X_child_nonzero + X_adult_nonzero + X_hospaccess_nonzero + X_popdensity_nonzero + X_housdensity_nonzero + X_flight_nonzero + X_vaxcovI_nonzero + X_vaxcovE_nonzero + X_H3_nonzero + X_humidity_nonzero 
}
...
ids <- modData %>% distinct(fips) %>% mutate(ID = seq_along(fips))
modData_full <- full_join(modData, ids, by = "fips")
