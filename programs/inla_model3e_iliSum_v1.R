...
## Name: Elizabeth Lee
## Date: 2/22/16
## Function: Model 3e inla - covariates only
...
## Data Source: 
## Notes: v1 - proportion in poverty
## 
## useful commands:
...
#### INLA modeling ################################
# Model 3e: covariates & unstructured spatial random effect
formula <- logy ~ -1 + f(ID, model = "iid") + I(X_poverty) # v1: poverty
...
for (s in seasons){
  ...
  # plot marginal posteriors for driver and sampling coefficients
  ...
  # plot choropleth of burden ratio (yhat_i/E) 
...
# setwd(dirname(sys.frame(1)$ofile))
# write_csv(modData_full, "testmethod_inlaData_model3e_v1.csv")
