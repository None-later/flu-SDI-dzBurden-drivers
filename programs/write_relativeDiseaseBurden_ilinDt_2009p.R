...
## Name: Elizabeth Lee
## Date: 4/19/17
## Function: 2009 pandemic version: write relative magnitude of disease burden with detrended ILIn (ilin.dt) Octfit data; data processing: Zip3-season combinations with equal or more consecutive weeks above the epidemic threshold in the non-flu period than the flu period are filtered out (see explore_fluSeasonDefinition_ILIn.R, Analysis 3 for additional details)
...
#   degree.var <- 2
  ...
  write.csv(dbMetrics.g2, file = sprintf('dbMetrics_periodicReg_%silinDt%s%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str, spatial$stringabbr), row.names=FALSE)
