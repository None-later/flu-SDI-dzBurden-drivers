...
## Name: Elizabeth Lee
## Date: 12/22/16
## Function: Model 7e v1: epidemic duration modeled as counts, test covariates, no spatial dependence
...
modCodeStr <- "7e_epiDur_v1-1"; testDataOn <- FALSE
rdmFx_RV <- "nu"
likString <- "normal"
...
#### Import and process data ####
  modData <- model7e_epiDur_v1(path_list) # with driver & sampling effort variables
  ...
  formula <- Y ~ -1 + 
    ...
    f(regionID_nonzero, model = "iid") + 
    f(season_nonzero, model = "iid") +
    intercept_nonzero + O_imscoverage_nonzero + O_careseek_nonzero + X_poverty_nonzero + X_vaxcovI_nonzero + X_H3A_nonzero + offset(logE_nonzero)
}
