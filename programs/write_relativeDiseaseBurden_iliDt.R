...
## Name: Elizabeth Lee
## Date: 10/20/15
## Function: write relative magnitude of disease burden with detrended ILI (ili.dt) Octfit data; data processing: Zip3-season combinations with equal or more consecutive weeks above the epidemic threshold in the non-flu period than the flu period are filtered out (see explore_fluSeasonDefinition_ILI.R, Analysis 3 for additional details)
...
setwd('../R_export')
data5 <- read_csv(sprintf('fullIndicAll_periodicReg_%siliDt%s%s_analyzeDB.csv', code, code2, code.str), col_names = T, col_types = list(zipname=col_character(), ili=col_double(), .fitted=col_double(), .se.fit=col_double(), ili.dt=col_double(), .fittedLoess=col_double(), .se.fitLoess=col_double()))
...
dbMetrics <- data5 %>% group_by(season, zipname) %>% filter(in.season) %>% summarize(iliDt.sum = sum(ili.dt, na.rm=T), iliDt.excess.BL = sum(ili.dt-.fitted, na.rm=T), iliDt.excess.thresh = sum(ili.dt-epi.thresh, na.rm=T), iliDt.peak = max(ili.dt, na.rm=T), epi.dur = sum(in.season))
...
setwd('../R_export')
write.csv(dbMetrics.g, file = sprintf('dbMetrics_periodicReg_%siliDt%s%s_analyzeDB.csv', code, code2, code.str), row.names=FALSE)
...
setwd('../R_export')
write.csv(dbMetrics.coords, file = sprintf('dbMetrics_periodicReg_%siliDt%s_analyzeDBdist.csv', code, code2, code.str), row.names=FALSE)
