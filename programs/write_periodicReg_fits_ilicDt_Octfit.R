...
## Name: Elizabeth Lee
## Date: 10/26/15
## Function: perform periodic regression on the detrended relative ilic (ilicDt)
...
write_periodicReg_fits_ilicDt_Octfit <- function(span.var, degree.var){
  #### header ####################################
  require(dplyr)
  require(ggplot2)
  require(broom)
  ...
  #### import data ####################################
  setwd('../R_export')
  ILI_full_df <- read_csv(file=sprintf('loess%s_allZip3Mods_ILIc.csv', code.str), col_types=list(zip3 = col_character(), ili = col_integer(), pop = col_integer(), cov_z.y = col_double(), alpha_z.y = col_double(), ILIc = col_double(), cov_below5 = col_logical(), .fitted=col_double(), .se.fit=col_double(), ilic.dt=col_double())) %>% 
  ...
  allMods <- ILI_full_df %>% filter(fit.week) %>% filter(Thu.week < as.Date('2009-05-01')) %>% 
    ...
    do(fitZip3 = lm(ilic.dt ~ t + cos(2*pi*t/52.18) + sin(2*pi*t/52.18), data = ., na.action=na.exclude))
  ...
  write.csv(allMods_fit_ILI, file=sprintf('periodicReg_%sallZip3Mods_ilicDt%s%s.csv', code, code2, code.str), row.names=FALSE)
  # write tidy coefficient dataset
  write.csv(allMods_tidy, file=sprintf('tidyCoef_periodicReg_%sallZip3Mods_ilicDt%s%s.csv', code, code2, code.str), row.names=FALSE)
  # write summary statistics for all models
  write.csv(allMods_glance, file=sprintf('summaryStats_periodicReg_%sallZip3Mods_ilicDt%s%s.csv', code, code2, code.str), row.names=FALSE)
