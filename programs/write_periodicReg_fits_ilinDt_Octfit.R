...
write_periodicReg_fits_ilinDt_Octfit <- function(span.var, degree.var, spatial){
  ...
  if (spatial$scale == 'zip3'){
    ILI_full_df <- read_csv(file=sprintf('loess%s_all%sMods_ILIn.csv', code.str, spatial$stringcode), col_types=list(zip3 = col_character(), ili = col_integer(), pop = col_integer(),  ILIn = col_double(), .fitted=col_double(), .se.fit=col_double(), ilin.dt=col_double(), ILIn = col_double())) %>%
  ...
  write.csv(perReg, file=sprintf('periodicReg_%sall%sMods_ilinDt%s%s.csv', code, spatial$stringcode, code2, code.str), row.names=FALSE)
  # write tidy coefficient dataset
  write.csv(tidCoef, file=sprintf('tidyCoef_periodicReg_%sall%sMods_ilinDt%s%s.csv', code, spatial$stringcode, code2, code.str), row.names=FALSE)
  # write summary statistics for all models
  write.csv(sumStats, file=sprintf('summaryStats_periodicReg_%sall%sMods_ilinDt%s%s.csv', code, spatial$stringcode, code2, code.str), row.names=FALSE)
