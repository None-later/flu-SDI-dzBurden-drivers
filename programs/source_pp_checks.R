...
export_predictiveChecks_seasIntensity <- function(path_csvExport_replicateData, path_csvExport_repSummData, modelOutput, modelData, modCodeStr){
    ...
    mutate(obs_rr = y1 - logE) %>%
    select(modCodeStr, ID, logE, y1, obs_rr)
    ...
    for(i in 1:nrow(modelData)){
        ...
        fitMarginals_rr[[i]] <- inla.tmarginal(function(x) (x - modelData2$logE[i]), modelOutput$marginals.fitted.values[[i]])
