...
# main_model_9e <- function(metriccode, season){
...
cd <- read_csv(paste0(herepath, "/../R_export/inlaModelData_import/inlaImport_model8e_epiDur_v7.csv"))
#### disease datasets ################################
...
forest_params <- ggplot(forestDat %>% filter(param %in% paste0("b", 1:19)), aes(x = label)) +
  ...
  scale_y_continuous("mean (95%CI)", limits = c(-.5,.5)) +
  theme_bw() +
