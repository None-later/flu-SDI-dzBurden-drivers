...
## Name: Elizabeth Lee
## Date: 2/17/16
## Function: INLA Model 3d - spatial CAR with sampling effort variables model
...
#### INLA modeling ################################
# Model 3d: null model
# Sampling effort & structured spatial random effect 
formula <- logy ~ -1 + f(ID, model = "besag", graph = path_adjMxExport_st) + I(O_imscoverage) + I(O_careseek) + I(O_insured)
...
for (s in seasons){
  ...
  plotDat <- left_join(modData_full, mod$summary.random$ID, by = "ID") %>%
    ...
    mutate(predMu = exp(logE + Predphi_mode)) %>%
