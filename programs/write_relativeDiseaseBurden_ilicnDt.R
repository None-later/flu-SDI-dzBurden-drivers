...
write_relativeDiseaseBurden_ilicnDt <- function(span.var, degree.var){
  ...
  setwd('../R_export')
  data5 <- read_csv(sprintf('fullIndicAll_periodicReg_%silicnDt%s%s_analyzeDB.csv', code, code2, code.str), col_names = T, col_types = list(zip3 = col_character(), ili = col_integer(), pop = col_integer(), cov_z.y = col_double(), alpha_z.y = col_double(), ILIc = col_double(), cov_below5 = col_logical(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ilicn.dt = col_double(), ILIcn = col_double()))
  ...
  dbMetrics <- data5 %>% group_by(season, zip3) %>% filter(in.season) %>% summarize(ilicnDt.sum = sum(ilicn.dt, na.rm=T), ilicnDt.excess.BL = sum(ilicn.dt-.fitted, na.rm=T), ilicnDt.excess.thresh = sum(ilicn.dt-epi.thresh, na.rm=T), ilicnDt.peak = max(ilicn.dt, na.rm=T), epi.dur = sum(in.season))
  
  # 8/6/15 create epidemic start timing and peak timing metrics
  # data processing in preparation for timing metrics
  data7 <- data5 %>% select(Thu.week, t, ilicn.dt, season, zip3, flu.week, in.season)
  ...
  createTiming2 <- data7 %>% mutate(ilicn.dt = as.numeric(ilicn.dt)) %>% group_by(season, zip3) %>% 
    filter(in.season) %>% 
  ...
  setwd('../R_export')
  write.csv(dbMetrics.g, file = sprintf('dbMetrics_periodicReg_%silicnDt%s%s_analyzeDB.csv', code, code2, code.str), row.names=FALSE)
  
  # save summary data to file with coords 
  # these data are used in "analyze_dbMetricsDistance_IR.R" for matrix correlations
  setwd('../R_export')
  write.csv(dbMetrics.coords, file = sprintf('dbMetrics_periodicReg_%silicnDt%s_analyzeDBdist.csv', code, code2, code.str), row.names=FALSE)
