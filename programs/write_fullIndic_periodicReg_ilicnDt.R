...
## Name: Elizabeth Lee
## Date: 10/26/15
## Function: write full datasets from periodic regression data of ilicn.dt, in preparation for write_relativeDiseaseBurden_ILIc.R: Zip3-season combinations with equal or more consecutive weeks above the epidemic threshold in the non-flu period than the flu period are filtered out (see explore_fluSeasonDefinition_ILIc.R, Analysis 3 for additional details)

## Filenames: periodicReg_%sallZip3Mods_ilicnDt_Octfit%s.csv
...
write_fullIndic_periodicReg_ilicnDt <- function(span.var, degree.var, spatial){
  ...
  data3 <- data2 %>% filter(incl.lm)  %>% mutate(epi.thresh = .fitted+(1.96*.se.fit)) %>% mutate(epi.week = ilicn.dt>epi.thresh)
  ...
  zip3s_with_epi <- full_join(dummy.flu, dummy.nf2, by=c("season", "scale")) %>% mutate(incl.analysis = consec.flu.epiweeks > consec.nf.epiweeks) %>% mutate(has.epi = (consec.flu.epiweeks>=4))
  ...
  write.csv(data5_write, file = sprintf('fullIndicFlu_periodicReg_%silicnDt%s%s_analyzeDB%s.csv', code, code2, code.str, spatial$stringabbr), row.names=FALSE)
  write.csv(data6_write, file = sprintf('fullIndicAll_periodicReg_%silicnDt%s%s_analyzeDB%s.csv', code, code2, code.str, spatial$stringabbr), row.names=FALSE)
