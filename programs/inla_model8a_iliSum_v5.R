...
modCodeStr <- "8a_iliSum_v5-1"; testDataOn <- FALSE
rdmFx_RV <- "phi"
likString <- "normal"
...
setwd('./countyCommuter_graph')
path_graphExport_cty <- paste0(getwd(), "/US_countyCommuter_edgelist.txt")
path_graphIdx_cty <- paste0(getwd(), "/US_countyCommuter_graph_index.csv")
...
path_list <- list(path_abbr_st = path_abbr_st,
                  ...
                  path_graphExport_cty = path_graphExport_cty,
...
#### Import and process data ####
  ...
  formula <- Y ~ -1 + 
    ...
    f(fips_nonzero, model = "iid") + 
    f(graphIdx_nonzero, model = "besag", graph = ctyCommuter_adjMx) +
    f(fips_st_nonzero, model = "iid") + 
    f(regionID_nonzero, model = "iid") + 
    f(season_nonzero, model = "iid") +
