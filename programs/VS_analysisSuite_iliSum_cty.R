...
setwd(dirname(sys.frame(1)$ofile))
setwd("../graph_outputs")
path_pltExport <- paste0(getwd(), "/VS_analysisSuite_iliSum_cty_noSt")
...
if("singleVarWrite" %in% analysesOn){
  ...
  #   }
    ...
    for (varInterest in varlist){
      ...
      coefRow <- model_singleVariable_inla_noSt(modDat, rCode, s, varInterest) # N.B. model includes intercept
