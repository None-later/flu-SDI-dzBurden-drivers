...
write_loess_fits_ILIc <- function(span.var, degree.var){
  #### header ####################################
  require(dplyr)
  require(ggplot2)
  require(broom)
...
#   degree.var <- 2
  ...
  allLoessMods_fit_ILI <- right_join((allLoessMods_aug %>% ungroup %>% select(-t)), (ilic_df %>% filter(Thu.week < as.Date('2009-05-01'))), by=c('Thu.week', 'zip3')) %>% 
    ...
    mutate(ilic.dt = ifelse(.fitted <= 1, ILIc, ILIc/.fitted)) 
  
  
  #### write data to file ####################################
  setwd('../R_export')
  # write fitted and original loess smoothed ILI data 
  write.csv(allLoessMods_fit_ILI, file=sprintf('loess%s_allZip3Mods_ILIc.csv', code.str), row.names=FALSE)
