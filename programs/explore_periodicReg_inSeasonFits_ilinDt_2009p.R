...
## Name: Elizabeth Lee
## Date: 4/19/17
## Function: 2009 pandemic version: 1. explore periodic regression fits of detrended ilin (ilin/fitted.loess), colored by in season classifications, through plotting
## Filenames: 
## Data Source: 
...
explore_periodicReg_inSeasonFits_ilinDt <- function(span.var, degree.var, spatial){
  ...
  if (spatial$scale == 'county'){
    data5 <- read_csv(sprintf('fullIndicAll_periodicReg_%silinDt%s%s%s%s%s_analyzeDB%s.csv', code, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str, spatial$stringabbr), col_names = T, col_types = list(fips = col_character(), ili = col_double(), pop = col_integer(), .fitted = col_double(), .se.fit = col_double(), .fittedLoess = col_double(), .se.fitLoess = col_double(), ilin.dt = col_double(), ILIn = col_double())) %>%
      rename(scale = fips)
  }
  
  #### 9/15/15 in.season fits ################################
  print(sprintf('plotting in season time series %s', code.str))
  dir.create(sprintf('../graph_outputs/explore_periodicReg_%sfits_ilinDt%s%s%s%s%s%s/inSeason', code, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str, spatial$stringabbr), showWarnings = FALSE)
  setwd(sprintf('../graph_outputs/explore_periodicReg_%sfits_ilinDt%s%s%s%s%s%s/inSeason', code, code2, spatial$servToggle, spatial$ageToggle, spatial$panToggle, code.str, spatial$stringabbr))
  ...
  for(i in indexes2){
    dummyplots <- ggplot(data_plot2 %>% filter(for.plot>= i & for.plot < i+num), aes(x=week, y=ilin.dt, group=scale)) +
      ...
      geom_line(aes(y=ifelse(pflu.week, 1, NA)), color = 'black') + 
